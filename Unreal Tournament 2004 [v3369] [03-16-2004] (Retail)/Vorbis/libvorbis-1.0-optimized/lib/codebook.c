 ...
 ********************************************************************
 ...
 last mod: $Id: codebook.c,v 1.1.1.1 2003/10/12 23:51:40 root Exp $
...
STIN long decode_packed_entry_number(codebook *_book, oggpack_buffer *b){
  register codebook *book = _book;
  register int  read=book->dec_maxlength;
  register long lo;
  register long hi;
  register long lok = oggpack_look(b,book->dec_firsttablen);
  ...
  {
    register ogg_uint32_t testword=bitreverse((ogg_uint32_t)lok);
    register ogg_uint32_t *codelist = book->codelist;
    register long hilo = hi-lo;
    while(hilo>1){
      register long p=hilo>>1;
...
long vorbis_book_decodevs_add(codebook *_book,float *a,oggpack_buffer *b,int n){
  register codebook *book = _book;
  register long dim = book->dim;
  register int step=n/dim;
  register long *entry = alloca(sizeof(*entry)*step);
  register float **t = alloca(sizeof(*t)*step);
  register int i;
  register int j;
  register int o;
  register float *aptr;
  register float *valuelist = book->valuelist;

  for (i = 0; i < step; i++) {
    entry[i]=decode_packed_entry_number(book,b);
    if(entry[i]==-1)return(-1);
    t[i] = valuelist+entry[i]*dim;
  }
  for(i=0,o=0;i<dim;i++,o+=step)
  {
    aptr = a + o;
    for (j=0;j<step;j++,aptr++)
      *aptr+=t[j][i];
  }
  return(0);
}
...
long vorbis_book_decodevv_add(codebook *book,float **a,long offset,int ch,
  ...
  if (ch == 2) {
    register float *left = a[0] + (offset>>1);
    register float *right = a[1] + (offset>>1);
    register float *max = a[0] + ((offset+n)>>1);
    ...
    if (dim == 2) {
      ...
      while (left < max) {
        register long extra = dim % 2;
        entry = decode_packed_entry_number(book,b);
        dim -= extra;
        if(entry==-1)return(-1);
        t = valuelist+entry*dim;
        for (j=0;j<dim;j+=2){
          *left+=*t;
          left++;
          t++;
    ...
    register float *mono = a[0] + offset;
    register float *max = a[0] + (offset+n);
    ...
    register float *t;
    register long i = offset/ch;
    register long max = (offset+n)/ch;
    register int chptr=0;
    while (i < max){
      entry = decode_packed_entry_number(book,b);
      if(entry==-1)return(-1);
      {
