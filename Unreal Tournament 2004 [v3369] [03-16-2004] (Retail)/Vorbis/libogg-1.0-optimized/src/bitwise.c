 ...
 ********************************************************************
  ...
  last mod: $Id: bitwise.c,v 1.1.1.1 2003/10/12 23:51:40 root Exp $
...
long oggpack_look(oggpack_buffer *_b,int _bits){
  /* forcing everything into registers gives a boost on non-x86 platforms. */
  register oggpack_buffer *b = _b;
  register int endbit = b->endbit;
  register int bits = _bits;
  register long endbyte = b->endbyte;
  register long storage = b->storage;
  register unsigned char *ptr = b->ptr;
  register unsigned long m=mask[bits];
  ...
  if (endbyte+4>=storage){
    /* not the main path */
    if(endbyte*8+bits>storage*8)return(-1);
  }
