...
class UT2StatsPrompt extends GUIPage;
...
function bool InternalOnClick(GUIComponent Sender)
{
	...
	if(Sender == Controls[1])
	{
		...
		SettingsPage = UT2SettingsPage(Controller.MenuStack[Controller.MenuStack.Length - 1]);
		assert(SettingsPage != None);
		
		GUITabControl(SettingsPage.Controls[1]).ActivateTabByName(SettingsPage.NetworkTabLabel,true);
	}
	else
