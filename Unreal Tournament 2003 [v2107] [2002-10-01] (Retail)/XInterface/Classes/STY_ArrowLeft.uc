// ====================================================================
//  Class:  XInterface.STY_ArrowLeft
//  Parent: XInterface.STY_RoundButton
//
//  <Enter a description here>
// ====================================================================

class STY_ArrowLeft extends STY_RoundButton;

defaultproperties
{
	KeyName="ArrowLeft"
	Images(0)=Material'InterfaceContent.Menu.ArrowBlueLeft'
	Images(1)=Material'InterfaceContent.Menu.ArrowBlueLeft'
	Images(2)=Material'InterfaceContent.Menu.ArrowBlueLeft'
	Images(3)=Material'InterfaceContent.Menu.ArrowBlueLeft'
	Images(4)=None
	ImgStyle(0)=ISTY_Scaled
	ImgStyle(1)=ISTY_Scaled
	ImgStyle(2)=ISTY_Scaled
	ImgStyle(3)=ISTY_Scaled
	ImgStyle(4)=ISTY_Scaled
}
